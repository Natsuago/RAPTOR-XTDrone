...

	{
		...
		if(_manual_control_input_sub.update(&manual_control_input)) {
			...
			if(_rlt_activ_src.get() == 0){
				...
				// PX4 v1.13 has no manual_control_setpoint.buttons bitmask.
				// Map button index [0..5] to aux1..aux6 channels.
				const int btn = _rlt_activ_btn.get();
				float aux_value = -1.0f;

				switch (btn) {
				case 0: aux_value = manual_control_input.aux1; break;
				case 1: aux_value = manual_control_input.aux2; break;
				case 2: aux_value = manual_control_input.aux3; break;
				case 3: aux_value = manual_control_input.aux4; break;
				case 4: aux_value = manual_control_input.aux5; break;
				case 5: aux_value = manual_control_input.aux6; break;
				default: aux_value = -1.0f; break;
				}

				next_command_active = aux_value >= 0.5f;
			}
		}
	}
	if(overwrite){
